# DFINITY基金会 
 ...
 ## 使命 
DFINITY为互联网计算机区块链贡献技术，为其生态圈提供支持，并倡导其作为一个技术栈的应用。DFINITY持有目前价值数十亿美元的[ICP代币](https://wiki.internetcomputer.org/wiki/ICP_token)资助，在需要时会出售这些代币来资助其运营。尽管DFINITY没有股东(shareholders)或成员（member），但DFINITY有一个使命:促进互联网计算机的发展和应用。 
